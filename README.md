...
│   └── vite.ts                      # Настройки Vite для разработки
...
Приложение построено по принципу **клиент-серверной архитектуры** с интеграцией AI:

```
┌─────────────────┐    HTTP/REST API    ┌─────────────────┐    AI API Pool    ┌─────────────────┐
│   React Client  │ ◄─────────────────► │  Express Server │ ◄──────────────► │   Google Gemini │
│   (Frontend)    │                     │   (Backend)     │                  │   AI (Multiple  │
│                 │                     │                 │                  │     Keys)       │
│ • Vite + React  │                     │ • Node.js       │                  └─────────────────┘
│ • Tailwind CSS  │                     │ • Express       │
│ • TanStack Query│                     │ • TypeScript    │
│ • Wouter Router │                     │ • Drizzle ORM   │
└─────────────────┘                     └─────────────────┘
        │                                        │
        ▼                                        ▼
┌─────────────────┐                     ┌─────────────────┐
│   Browser Cache │                     │  PostgreSQL DB  │
│   (TanStack     │                     │  (Neon Database)│
│    Query)       │                     │                 │
└─────────────────┘                     └─────────────────┘
```
...
async function processChunksInParallel(chunks, checklist, risks, perspective) {
  // Батчевая обработка (по 3 чанка одновременно)
...
### v2.0.0 - Многоэтапный анализ и исправление противоречий (Декабрь 2024)
...
- **Drizzle ORM + PostgreSQL**: Переход на надежную систему хранения данных
- **Система кэширования**: Сохранение результатов анализа для повторного использования
- **Обработка ошибок**: Многоуровневая система с детальным логированием
- **Оптимизация производительности**: Контролируемые задержки и балансировка нагрузки
...
1. **Сообщите об ошибке** - создайте issue с подробным описанием
...
- [Drizzle ORM](https://orm.drizzle.team/) за современный подход к работе с базами данных
- Юридическому сообществу за обратную связь и тестирование

## 🔗 Полезные ссылки

- **Телеграм-канал**: [AI Скрепка: Связь Права и Технологий](https://t.me/+plgepYs_y0M3ODJi)
- **Google AI Studio**: [Получить API ключ](https://aistudio.google.com/)
- **Neon Database**: [Бесплатная PostgreSQL база данных](https://neon.tech/)
- **Документация Gemini API**: [Официальная документация](https://ai.google.dev/docs)
