...
- **Интегрированный анализ неоднозначностей**: Неоднозначные условия выделяются прямо в тексте договора с желтым фоном
- **Адаптивная оптимизация токенов**: Динамическое определение оптимального лимита токенов (6000-8192) на основе длины договора
...
### v1.2.0 - Оптимизация анализа и UX улучшения
- **Интегрированный анализ неоднозначностей**: Неоднозначные условия теперь выделяются прямо в тексте договора с категорией "ambiguous" (желтый фон) вместо отдельного блока
- **Адаптивная система токенов**: Реализована умная оптимизация выходных токенов с 4 стратегиями:
  - КОМПАКТНЫЙ (<5K символов, 6000 токенов)
  - СТАНДАРТНЫЙ (5-15K, 7500 токенов) 
  - РАСШИРЕННЫЙ (15-30K, 8000 токенов)
  - МАКСИМАЛЬНЫЙ (>30K, 8192 токенов)
...
- **Детальный мониторинг**: Добавлен мониторинг экономии токенов и предотвращения обрезания ответов
